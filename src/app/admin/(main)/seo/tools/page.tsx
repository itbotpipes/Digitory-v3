...
import { FileText, Save, RefreshCw, Bot } from 'lucide-react';
...
export default function SeoTools() {
  ...
  const fetchLlm = async () => {
    try {
      const token = localStorage.getItem('admin_token') || '';
      const res = await api.get('/llm', token);
      if (res.data) setLlmTxt(res.data.content);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const saveLlm = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token') || '';
      await api.post('/llm', { content: llmTxt }, token);
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 animate-fade-in">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Sitemap Section */}
        <section className="bg-zinc-50 dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800/80 rounded-3xl p-8 shadow-sm flex flex-col justify-between">
          <div>
        ...
        <section className="bg-zinc-50 dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800/80 rounded-3xl p-8 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <textarea 
              ...
              className="w-full h-52 bg-zinc-900 dark:bg-black border border-zinc-800 rounded-2xl p-5 font-mono text-sm text-green-400 outline-none focus:border-[#FF4F18] focus:ring-1 focus:ring-[#FF4F18] resize-none mb-8 custom-scrollbar shadow-inner"
      ...
      {/* llm.txt Section (Positioned Below) */}
      <section className="bg-zinc-50 dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800/80 rounded-3xl p-8 shadow-sm flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1">Guide AI models & LLM agents with custom site context documentation.</p>
            </div>
          </div>
          ...
          <textarea 
            ...
            className="w-full h-72 bg-zinc-900 dark:bg-black border border-zinc-800 rounded-2xl p-5 font-mono text-sm text-amber-300 outline-none focus:border-[#FF4F18] focus:ring-1 focus:ring-[#FF4F18] resize-none mb-6 custom-scrollbar shadow-inner"
        ...
        <div className="flex justify-end">
          <button 
            onClick={saveLlm}
            disabled={savingLlm}
            className="w-full sm:w-auto px-8 flex items-center justify-center gap-2 bg-[#FF4F18] text-white font-bold py-3.5 rounded-xl hover:bg-[#E03F0D] transition-all duration-200 shadow-[0_4px_14px_rgba(255,79,24,0.35)] hover:shadow-[0_6px_20px_rgba(255,79,24,0.4)] disabled:opacity-50 transform hover:-translate-y-0.5"
          >
